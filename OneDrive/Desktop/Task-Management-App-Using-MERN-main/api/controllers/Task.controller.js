...
export const createTask = async (req, res) => {
    try {
        const { title, description, dueDate } = req.body  // ✅ dueDate add
        const newTask = new TaskModel({
            title,
            description,
            dueDate  // ✅ save kar rahe hain
        })
        await newTask.save()
...
export const updateTask = async (req, res) => {
    try {
        const { taskid } = req.params
        const { title, description, status, dueDate } = req.body  // ✅ dueDate add

        const taskData = await TaskModel.findByIdAndUpdate(
            taskid,
            { title, description, status, dueDate }, // ✅ update kar rahe hain
            { new: true }
        )
